...
description: "App review rejections are predictable and mostly avoidable. The eight reasons we see most, plus the pre-submission checklist we run before every release."
date: 2026-08-15
...
**8. Not enough app.** Apple rejects apps that are a repackaged website or offer little beyond a static brochure — which is worth settling before you build, when you ask [what the app does that a website cannot](/blog/cross-platform-vs-native). If your app is a wrapper, expect a fight.
...
Google Play review is faster and more automated, but has its own tripwires: the Data Safety form, target API level deadlines, and background location justifications — [notification permissions](/blog/push-notifications-that-work) deserve the same care. Budget for a policy review rather than assuming it will sail through.

Rejections aside, plan the release itself around [a tight first-version scope](/blog/mobile-app-mvp-scope) — unfinished features are the most common reason a build is not submission-ready.
